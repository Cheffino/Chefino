...
struct GardeMangerRecipesView: View {
    ...
    @FetchRequest(
        ...
        predicate: NSPredicate(format: "category == %@", "Garde Manger")
    ...
    var body: some View {
        VStack {
            ...
            if isGridView {
                GardeMangerGridView(recipes: filteredRecipes)
            } else {
                GardeMangerListView(recipes: filteredRecipes)
            }
        }
        .navigationTitle("Garde Manger Recipes")
        .onAppear {
    ...
    func fetchRecipesFromFirebase() {
        ...
        db.collection("recipes").whereField("category", isEqualTo: "Garde Manger").getDocuments { (querySnapshot, error) in
            if let error = error {
                ...
                for document in querySnapshot!.documents {
                    ...
                    if let recipe = Recipe(dictionary: data) {
                        ...
                        newRecipe.category = "Garde Manger"
                    }
                }
...
struct GardeMangerListView: View {
...
struct GardeMangerGridView: View {
    let recipes: [DishEntity]
    let columns = [
        GridItem(.flexible()),
        GridItem(.flexible())
...
struct GardeMangerRecipesView_Previews: PreviewProvider {
    static var previews: some View {
        GardeMangerRecipesView()

...
struct EntrimetrierRecipesView: View {
    ...
    @FetchRequest(
        ...
        predicate: NSPredicate(format: "category == %@", "Entrimetrier")
    ...
    var body: some View {
        VStack {
            ...
            if isGridView {
                EntrimetrierGridView(recipes: filteredRecipes)
            } else {
                EntrimetrierListView(recipes: filteredRecipes)
            }
        }
        .navigationTitle("Entrimetrier Recipes")
        .onAppear {
    ...
    func fetchRecipesFromFirebase() {
        ...
        db.collection("recipes").whereField("category", isEqualTo: "Entrimetrier").getDocuments { (querySnapshot, error) in
            if let error = error {
                ...
                for document in querySnapshot!.documents {
                    ...
                    if let recipe = Recipe(dictionary: data) {
                        ...
                        newRecipe.category = "Entrimetrier"
                    }
                }
...
struct EntrimetrierListView: View {
...
struct EntrimetrierGridView: View {
    let recipes: [DishEntity]
    let columns = [
        GridItem(.flexible()),
        GridItem(.flexible())
...
struct EntrimetrierRecipesView_Previews: PreviewProvider {
    static var previews: some View {
        EntrimetrierRecipesView()
